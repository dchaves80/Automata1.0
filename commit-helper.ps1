# 🎯 Automata Factory - Commit Helper
# Script interactivo para commits siguiendo las convenciones del proyecto

Write-Host "🏭 Automata Factory - Commit Helper" -ForegroundColor Cyan
...
# Definir tipos de commit con emojis usando códigos Unicode
$commitTypes = @{
    "1"  = @{ emoji = "`u{2728}"; code = "feat"; desc = "Nueva funcionalidad" }
    "2"  = @{ emoji = "`u{1F41B}"; code = "fix"; desc = "Correccion de bugs" }
    "3"  = @{ emoji = "`u{1F4DA}"; code = "docs"; desc = "Documentacion" }
    "4"  = @{ emoji = "`u{1F527}"; code = "config"; desc = "Configuracion" }
    "5"  = @{ emoji = "`u{1F6A7}"; code = "wip"; desc = "Work in progress" }
    "6"  = @{ emoji = "`u{1F389}"; code = "milestone"; desc = "Fase completada" }
    "7"  = @{ emoji = "`u{1F5C4}"; code = "db"; desc = "MongoDB/Base de datos" }
    "8"  = @{ emoji = "`u{1F3AE}"; code = "unity"; desc = "Unity/Cliente" }
    "9"  = @{ emoji = "`u{1F310}"; code = "api"; desc = "API/Backend" }
    "10" = @{ emoji = "`u{1F484}"; code = "ui"; desc = "UI/Interfaz" }
    "11" = @{ emoji = "`u{1F528}"; code = "refactor"; desc = "Refactoring" }
    "12" = @{ emoji = "`u{1F6A8}"; code = "test"; desc = "Testing" }
    "13" = @{ emoji = "`u{1F512}"; code = "security"; desc = "Seguridad" }
    "14" = @{ emoji = "`u{1F40E}"; code = "perf"; desc = "Performance" }
    "15" = @{ emoji = "`u{1F691}"; code = "hotfix"; desc = "Hotfix critico" }
    "16" = @{ emoji = "`u{1F6E0}"; code = "tool"; desc = "Herramientas/Scripts" }
}
...
$branches = git branch -a | ForEach-Object { $_.Trim() -replace '^\*\s*', '' -replace '^remotes/origin/', '' } | Sort-Object | Get-Unique
